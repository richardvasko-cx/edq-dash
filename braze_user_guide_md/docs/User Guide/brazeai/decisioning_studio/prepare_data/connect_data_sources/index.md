...
If there is customer data that you want to use for Decisioning Studio that is not currently stored in the user profile or custom attributes, the recommended approach is to use [Braze Cloud Data Ingestion](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion) to ingest data from other sources.
...
For the full list of supported sources, see [Cloud Data Ingestion](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion).
...
To streamline this process, create a list of Braze user profile attributes that you think best represent your customers' behaviors that should be used in Decisioning Studio (see the [list of available fields](https://www.braze.com/docs/api/endpoints/export/user_data/post_users_segment/#fields-to-export)). Your services team can also help you conduct discovery sessions to decide which fields are most appropriate for AI Decisioning.

Other options for sending data include:

- Sending Braze custom events via the SDK
- Sending events using the REST endpoint ([`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track))
