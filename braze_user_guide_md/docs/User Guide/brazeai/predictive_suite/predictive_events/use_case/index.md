...
2. He [creates a new event prediction](https://www.braze.com/docs/user_guide/brazeai/predictive_suite/predictive_events/creating_an_event_prediction/) and names it "Upgrade to Pro in 7 days"
...
![Prediction Settings showing the definition, window, audience, and update schedule for the prediction.](https://www.braze.com/docs/assets/img/ai_use_cases/prediction_settings.png?f25209f7875e7c8c66aea485fc1cbe43)

## Step 2: Segment users based on upgrade probability

After training is complete, Braze assigns an [Event Likelihood Score](https://www.braze.com/docs/user_guide/brazeai/predictive_suite/predictive_events/analytics#purchase_score) (0-100) to each eligible user. Jordan uses this score to create actionable segments—one for high-intent users who may not need a discount, and another for users who likely won’t convert without support.

1. Jordan navigates to Segments in Braze.
2. He creates two [segments](https://www.braze.com/docs/user_guide/audience/segments/creating_a_segment/) using the [Event Likelihood Score filter](https://www.braze.com/docs/user_guide/audience/segments/segmentation_filters#event-likelihood-score) and selects the prediction he created. The two segments are:
...
![Segment builder with two filters for Event Likelihood Score.](https://www.braze.com/docs/assets/img/ai_use_cases/event_likelihood_score.png?9a612fc84d7aa2ed4b3d53621ebda129)
...
Jordan [creates a Canvas](https://www.braze.com/docs/user_guide/messaging/canvas/create_a_canvas/) that splits the experience based on the segments he just built. He adds an Audience Paths step to target:
...
![Canvas Audience Path with four paths for each intent type.](https://www.braze.com/docs/assets/img/ai_use_cases/canvas_paths_by_intent.png?39543b753a83b24c70579e2cbbec73d3)
...
The [funnel report](https://www.braze.com/docs/user_guide/analytics/reports/funnel_reports/) also shows a clear reduction in drop-off across key steps, particularly for low-intent users who received personalized messaging. More users are opening, clicking, and upgrading—proving the value of intent-based targeting.
