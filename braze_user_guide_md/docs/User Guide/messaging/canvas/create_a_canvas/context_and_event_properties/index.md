...
> This reference article covers information about `context` and `event_properties`, including when to use each property and the differences in behavior. <br><br> For information about custom event properties in general, check out [Custom events properties](https://www.braze.com/docs/user_guide/data/activation/events/custom_events/custom_event_properties/).
...
- Cannot be used in scheduled full steps other than the first full step of an action-based Canvas. However, if a user is using a [Canvas component](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/about/), the behavior follows the current Canvas workflow rules for `event_properties`.
...
Context is configured in the [Entry Schedule](https://www.braze.com/docs/user_guide/messaging/canvas/create_a_canvas/#step-12-determine-your-canvas-entry-schedule) step of creating a Canvas and correspond to when a user enters a Canvas. Context can also be referenced in any Message step.
