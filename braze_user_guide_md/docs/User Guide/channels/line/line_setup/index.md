...
**Setting `native_line_id`**: You can set `native_line_id` by sending user updates to Braze (for example, with the [`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track/) endpoint, [CSV import](https://www.braze.com/docs/user_guide/audience/manage_audience/import_users#csv-import), or [Cloud Data Ingestion](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/)). If your client-side SDK doesn’t have a dedicated field for `native_line_id`, send it in server-side user updates using one of these methods.
...
You can import or update users using any of the methods that Braze supports, including the [`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track/) endpoint, [CSV import](https://www.braze.com/docs/user_guide/audience/manage_audience/import_users#csv-import), or [Cloud Data Ingestion](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/). 
...
1. In LINE, go the **Messaging API** tab and edit your **Webhook settings**:
   ...
   - Turn on **Use webhook** and **Webhook redelivery**. <br><br> ![Webhook settings page to verify or edit the webhook URL, toggling on or off "Use webhook", "Webhook redelivery", and "Error statistics aggregation".](https://www.braze.com/docs/assets/img/line/webhook_settings.png?72a60278af8536b065470808c59a19c3){: style="max-width:70%;"}
...
![Response settings page with toggles for how your account will handle chats.](https://www.braze.com/docs/assets/img/line/response_settings.png?b9f565b69d65ae417331c161bdd12621){: style="max-width:80%;"}
...
If you want to add IP whitelisting in your LINE account, add all of the IP addresses listed for your cluster in [IP allowlisting](https://www.braze.com/docs/user_guide/channels/webhooks/create_a_webhook/#ip-allowlisting) to your allowlist.
...
![LINE messaging integration page with LINE integration section.](https://www.braze.com/docs/assets/img/line/integration.png?59500ac7fbe1f3371ecbe1b759931f22){: style="max-width:80%;"}
...
![LINE subscription groups section displaying one subscription group for the "LINE" channel.](https://www.braze.com/docs/assets/img/line/line_subscription_groups.png?147cb1b1901e690be3b7e7c277c16d73){: style="max-width:80%;"}
...
When a LINE user is identified in your application through [user reconciliation](#user-id-reconciliation) or other means, you can target a potential unidentified user profile in Braze using the [`/users/identify`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify/) endpoint. Every unidentified user profile with a `native_line_id` also has a user alias `line_id` that can be used to target the user profile to identify.
...
You can update LINE users that are known in your application through the [`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track/) endpoint by passing their external identifiers and `native_line_id`. If an unidentified user profile already exists for a user and the same `native_line_id` is added to a different user profile through `/users/track`, it will inherit all the subscription states of the unidentified user profile. However, duplicate user profiles will exist with the same `native_line_id`. Any subsequent subscription updates from event updates will update all profiles accordingly. 
...
To learn more about managing duplicate users in Braze, see [Duplicate Users](https://www.braze.com/docs/user_guide/audience/manage_audience/merge_duplicate_users/).
...
3. The user is identified as having the LINE ID through [user reconciliation](#user-id-reconciliation).
  - The anonymous user profile can become identified using the [`/users/identify`](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify/) endpoint. Subsequent updates (through the [`/users/track`](https://www.braze.com/docs/api/endpoints/user_data/post_user_track/) endpoint, [CSV import](https://www.braze.com/docs/user_guide/audience/manage_audience/import_users#csv-import), or [Cloud Data Ingestion](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/)) to this user profile can target the user by this known `external_id`.
...
5. Send the new or updated user information to Braze using the [`/user/track` endpoint](https://www.braze.com/docs/api/endpoints/user_data/post_user_track#track-users/), [CSV import](https://www.braze.com/docs/user_guide/audience/manage_audience/import_users#csv-import), or [Cloud Data Ingestion](https://www.braze.com/docs/user_guide/data/unification/cloud_ingestion/).
...
1. Create an action-based Canvas that is based on a subscription state change and triggers when a user subscribes to your LINE channel.<br>![Canvas that triggers when a user subscribes to the LINE channel.](https://www.braze.com/docs/assets/img/line/account_link_1.png?fa9df4bf87645711d0e42de47e0a246c)
...
4. (Optional) Create an action-based campaign or Canvas that triggers when the LINE user is identified to send the user their coupon code. <br>![Action-based campaign that triggers when the LINE user is identified.](https://www.braze.com/docs/assets/img/line/account_link_2.png?55f4024f01deedda5c8dc0db057016f1)
...
4. The anonymous user profile that was created is identified and is merged through the [/users/identify endpoint](https://www.braze.com/docs/api/endpoints/user_data/post_user_identify/) onto the user’s known user profile. The known user profile now contains the LINE ID and has a subscription status of `subscribed`.
...
1. Set up a Canvas that returns a user's Braze user ID on a specific trigger word. <br><br>Example trigger <br><br>![Trigger to send the campaign to users who sent an inbound LINE to a specific subscription group.](https://www.braze.com/docs/assets/img/line/trigger.png?d4631ce7c3e024a074c53c1535e8f8de){: style="max-width:80%;"}<br><br>Example message<br><br>![LINE message stating the Braze user ID.](https://www.braze.com/docs/assets/img/line/message.png?18abe976b50fd16e715822130400733f){: style="max-width:40%;"}<br><br>
