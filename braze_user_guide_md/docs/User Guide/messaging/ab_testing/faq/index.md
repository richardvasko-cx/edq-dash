...
Variant assignment is randomized on each send, so the actual split may not match your configured percentages exactly—especially with smaller sample sizes. For more information, see [Variant distribution](https://www.braze.com/docs/user_guide/messaging/ab_testing/concepts/variant_distribution/).
...
Users are randomly assigned to a particular variant before receiving the campaign for the first time. Each successive time the campaign is received (or the user re-enters a Canvas variant), they receive the same variant unless the variant percentages are modified. If the variant percentages change, users may be redistributed to other variants. Users stay in these variants until percentages are modified again. Users are redistributed only for the variants that were edited.
