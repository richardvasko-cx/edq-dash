...
A snapshot is static and aggregated. It reflects the cumulative result of all changes up to that moment. This is best for customer profiles, computed features (for example, "days since last purchase," "loyalty tier," "churn score").
...
- **Scope:** Every update must include all relevant customers, including those who had no event.
...
### Query snapshot data for daily delivery
...
An event stream records discrete actions as they happen. It answers the question: "What did this customer do, and when?" An event stream is ideal for raw, immutable, incremental, and chronological data. Each record represents one thing that happened at a specific time. For example, use this data stream for activation records, engagement logs (opens, clicks), conversion events, or coupon redemptions.
...
### Query event data for daily delivery
...
### Update snapshot data on an event trigger
