...
The Braze web SDK is initialized when a user submits a form on a landing page. Prior to form submission, no personal data is collected and the SDK doesn't actively track users. After initialization is completed, the SDK doesn't store any data in the browser (such as cookies, local storage, or others). 

The Braze web SDK is initialized immediately when a user navigates to the landing page through a link generated by a `{% landing_page_url %}` Liquid tag in a Braze message.
...
- Trigger the [`/users/merge` endpoint](https://www.braze.com/docs/api/endpoints/user_data/post_users_merge/) when a landing page form is submitted to merge the new profile with an existing profile.
- Schedule [bulk merging](https://www.braze.com/docs/user_guide/audience/manage_audience/merge_duplicate_users/#bulk-merging) to periodically merge duplicate profiles based on matching identifiers.
