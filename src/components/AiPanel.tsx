...
type PanelFormat = 'Standard' | 'Shorter' | 'Expand' | 'Data +';
interface ConvSummary { id: string; title: string; timestamp: string; preview: string; source: string; }

const PANEL_FORMAT_OPTIONS: PanelFormat[] = ['Standard', 'Shorter', 'Expand', 'Data +'];
...
/** Cycles short status words with a bouncy bottom-up motion while Gemini thinks.
 *  Time-based so remounting mid-generation shows the correct word, not always word 0. */
...
export default function AiPanel({ isDark, screenContext, chatHistory, onUpdateHistory, onClose, onOpenArticle, selectorActive, onSelectorToggle, pendingChip, onChipConsumed, onLoadingChange, onChipsChange, pendingQuery, pendingQueryDisplay, onPendingQueryConsumed, searchQuery = '', onSearchQueryChange, searchResults = [], onSelectSearchResult, onRunAction }: AiPanelProps) {
  ...
  const REFINE_LABEL: Record<'shorter' | 'technical' | 'data', string> = {
    shorter: 'Make this shorter',
    technical: 'Expand the context and provide technical detail',
    data: 'Show the supporting data',
  };
  const refineLast = async (refineMode: 'shorter' | 'technical' | 'data') => {
    if (isLoading) return;
    ...
    const selectionMsg: ChatMessage = { role: 'user', content: REFINE_LABEL[refineMode], timestamp: Date.now() };
    ...
    const apiMsgs = finalHistory.map((m: ChatMessage) => ({ role: m.role, content: m.content, chips: m.chips?.length ? m.chips : null, articles: m.articles?.length ? m.articles : null, actions: m.actions?.length ? m.actions : null }));
  ...
  const sendMessage = async (query?: string, overrideChips?: PanelChip[], baseHistoryOverride?: ChatMessage[], edited?: boolean, displayContent?: string) => {
    ...
    const finalHistory: ChatMessage[] = [...next, { role: 'assistant' as const, content: finalAnswer, timestamp: Date.now(), articles, article: articles[0] ?? null, actions: ensureLocalAppActions(finalAnswer, actions), followups: followups.length ? followups : undefined }];
    onUpdateHistory(finalHistory);
    setIsLoading(false); onLoadingChange?.(false);

    // Auto-save conversation
    const apiMsgs = finalHistory.map((m: ChatMessage) => ({ role: m.role, content: m.content, chips: m.chips?.length ? m.chips : null }));
  ...
  const t = isDark
    ...
    : {
        ...
        inputWrap: 'bg-[#F1F3F4] border-black/8',
  ...
  return (
    <div className="relative w-full h-full flex flex-col shrink-0">
      <div className={cn('relative flex-1 rounded-[16px] flex flex-col overflow-hidden', t.shell, isDark ? '' : 'border border-black/8')}>
        ...
        <div className={cn('flex items-center gap-2.5 px-4 pt-4 pb-3 border-b shrink-0', t.border)}>
          {historyView ? (
            ...
            <>
              ...
              <span className={cn('text-[15px] font-bold flex-1', t.headTitle)}>Gemini</span>
              <span className="text-[10px] font-semibold px-1.5 py-0.5 bg-[#1A73E8]/20 text-[#1A73E8] dark:text-[#74BBFF] rounded-full border border-[#1A73E8]/30">Alpha</span>
              <button type="button" onClick={() => { onUpdateHistory([]); conversationIdRef.current = null; }} className={cn('w-6 h-6 flex items-center justify-center rounded-full transition-colors', t.subtle, t.subtleHover)} title="New chat">
                <span className="material-symbols-outlined text-[16px]">edit_square</span>
              </button>
              <button type="button" onClick={onClose} className={cn('w-6 h-6 flex items-center justify-center rounded-full transition-colors', t.subtle, t.subtleHover)}>
                <span className="material-symbols-outlined text-[14px]">close</span>
              </button>
            </>
          )}
                  ...
                  <div key={i} data-gem-msg={i} className={cn('flex flex-col scroll-mt-3', msg.role === 'user' ? 'items-end' : 'items-start')}>
                    {msg.role === 'user' ? (
                      ...
                      <div className="group w-full">
                        ...
                        {/* Suggested Braze User Guide articles for this answer (up to 3). */}
                        {(() => {
                          const cards: GuideArticle[] = (msg.articles && msg.articles.length ? msg.articles : (msg.article ? [msg.article] : [])).slice(0, 3);
                            ...
                            <div className="mt-2 ml-[22px] flex flex-col gap-1.5" style={{ width: 'calc(100% - 22px)' }}>
                              {cards.length > 1 && (
                                <span className={cn('text-[8.5px] font-black uppercase tracking-widest pl-0.5', isDark ? 'text-white/35' : 'text-black/35')}>
                                  Suggested guides · {cards.length}
                                </span>
                              )}
                              {cards.map((c, ci) => (
                                <button type="button"
                                  key={ci}
                                  onClick={() => onOpenArticle(c.path)}
                                  className={cn(
                                    'flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition-colors group',
                                    isDark
                                      ? 'bg-white/5 border-white/8 hover:bg-white/8'
                                      : 'bg-[#F1F3F4] border-black/5 hover:bg-[#E8F0FE]'
                                  )}
                                >
                                  <DocSparkIcon className={cn('w-[18px] h-[18px] shrink-0', isDark ? 'text-[#74BBFF]' : 'text-[#1A73E8]')} />
                                  <span className="flex-1 min-w-0">
                                    <span className={cn('block text-[8.5px] font-bold uppercase tracking-wide', isDark ? 'text-white/40' : 'text-black/40')}>
                                      {cards.length > 1 ? c.section : `Suggested guide · ${c.section}`}
                                    </span>
                                    <span className={cn('block text-[12px] font-semibold truncate', isDark ? 'text-white/85' : 'text-[#1D192B]')}>
                                      {c.title}
                                    </span>
                                  </span>
                  ...
                  <div className="flex gap-2 items-start w-full">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-2 pt-1">
                      <>
                        ...
                        <ThinkingStatus className={cn('mt-0.5', isDark ? 'text-[#74BBFF]/70' : 'text-[#1A73E8]/70')} startTime={loadingStartRef.current} />
                      </>
                    </div>
                  </div>
                )}
          ...
          <AnimatePresence>
            {!historyView && !isEmpty && showScrollDown && (
              <motion.button type="button"
                ...
                className={cn(
                  'absolute right-3 bottom-[72px] z-20 w-8 h-8 rounded-full flex items-center justify-center border shadow-md transition-colors',
          ...
          <AnimatePresence>
            {historyView && (
              <motion.div
                ...
                className={cn('absolute inset-0 z-20 overflow-y-auto custom-scrollbar px-3 py-3', isDark ? 'bg-[#13141F]' : 'bg-white')}
              >
        ...
        {!historyView && <div className={cn('px-4 pb-4 pt-2 shrink-0 border-t', t.border)}>
          ...
          <div className="relative">
                ...
                >
                  {PANEL_FORMAT_OPTIONS.map(fmt => (
                    <button type="button"
                      ...
                      className={cn(
                        'w-full flex items-center justify-between px-4 py-2 text-[12px] font-medium transition-colors',
                        isDark ? 'text-white/90 hover:bg-white/8' : 'text-[#1F1F1F] hover:bg-[#F1F3F4]',
                        panelFormat === fmt && 'text-[#1A73E8]',
                      )}
                    >
                      <span>{fmt}</span>
                      {panelFormat === fmt && <span className="material-symbols-outlined text-[#1A73E8] text-[14px]">check</span>}
                    </button>
                  ))}
                </motion.div>
              )}
            ...
            <div className={cn('flex items-center gap-1 rounded-[28px] px-2.5 py-1.5 border min-w-0 overflow-hidden', t.inputWrap)}>
              ...
              <div className="flex-1 min-w-0 flex items-center">
                <input
                  type="text"
                  className={cn('w-full min-w-0 bg-transparent outline-none text-[13px] truncate', t.inputText)}
                  placeholder="Ask Gemini or type to search"
                  value={input}
                  onChange={e => updateInput(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter' && !isLoading && (input.trim() || panelChips.length > 0)) sendMessage(); }}
                  disabled={isLoading}
                />
              </div>
              {isLoading ? (
                <button type="button"
                  ...
                  className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 transition-all bg-[#0b57d0] text-white hover:bg-[#0842a0] hover:scale-105"
