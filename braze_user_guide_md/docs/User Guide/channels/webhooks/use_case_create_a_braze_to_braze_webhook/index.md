...
> Braze-to-Braze webhooks let you call the [Braze REST API](https://www.braze.com/docs/api/basics/) from within Braze using a [Webhook](https://www.braze.com/docs/user_guide/channels/webhooks/create_a_webhook/) in a [Campaign](https://www.braze.com/docs/user_guide/messaging/campaigns/) or [Canvas](https://www.braze.com/docs/user_guide/messaging/canvas/). Use this for orchestration tasks like triggering an [API-triggered Canvas](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/). For updating [User attributes](https://www.braze.com/docs/user_guide/data/activation/attributes/custom_attributes/), [Custom events](https://www.braze.com/docs/user_guide/data/activation/events/custom_events/), or [Purchases](https://www.braze.com/docs/user_guide/data/activation/events/purchase_events/) from Canvas, use [User Update](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/user_update/) instead. It's designed for user profile changes and processes updates more efficiently.
...
Add [User Update](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/user_update/) to your Canvas to update a user's attributes, events, and purchases using a JSON composer.
...
To create a Braze-to-Braze webhook, you need an [API key](https://www.braze.com/docs/api/api_key/) with permissions for the endpoint you want to reach. For example, to trigger an API-triggered Canvas, you need an API key with the `canvas.trigger.send` permission.

## Setting up your Braze-to-Braze webhook

The general workflow for creating a Braze-to-Braze webhook follows these steps:

1. [Create a webhook](https://www.braze.com/docs/user_guide/channels/webhooks/create_a_webhook/) as a campaign or Canvas component. 
...
- **Webhook URL:** Your [REST endpoint URL](https://www.braze.com/docs/user_guide/administer/personal/sdk_endpoints/) followed by `/canvas/trigger/send`. For example, for the `US-06` instance, the URL would be `https://rest.iad-06.braze.com/canvas/trigger/send`.
...
![Request headers for the webhook showing Authorization and Content-Type fields in the Braze dashboard.](https://www.braze.com/docs/assets/img_archive/webhook_settings.png?e1b973127166559a0379333e60256be0){: style="max-width:70%;"}

#### Request body

Add your `/canvas/trigger/send` request in the text field. For details, see [Sending Canvas messages via API-triggered delivery](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_triggered_canvases/). The following is an example of the request body for this endpoint, where `your_canvas_id` is the Canvas ID from your second Canvas:
...
- **User updates:** For updating user profiles from Canvas (attributes, events, purchases), use [User Update](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/user_update/) instead of Braze-to-Braze webhooks for better efficiency and cost-effectiveness.
- Braze-to-Braze webhooks are subject to endpoint [Rate limits](https://www.braze.com/docs/api/api_limits/).
- Updates to the user profile incur [Data points](https://www.braze.com/docs/user_guide/data/infrastructure/data_points/) that count toward your overall consumption, while triggering another message through the messaging endpoints does not.
- To target [Anonymous users](https://www.braze.com/docs/user_guide/data/unification/user_data/user_profile_lifecycle#anonymous-user-profiles), use `braze_id` instead of `external_id` in the request body of your webhook.
- You can save your Braze-to-Braze webhook as a [webhook template](https://www.braze.com/docs/user_guide/messaging/templates/webhook_templates/) for reuse.
- You can check the [Message Activity Log](https://www.braze.com/docs/user_guide/administer/global/workspace_settings/logs_and_alerts/message_activity_log/) to view and troubleshoot webhook failures.
