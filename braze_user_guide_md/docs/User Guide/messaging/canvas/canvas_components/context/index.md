...
![A Context step as the first step of a Canvas.](https://www.braze.com/docs/assets/img/context_step3.png?fa6617cfab3adb369b499fdc7aa9d553){: style="float:right;max-width:40%;margin-left:15px;"}
...
When referencing context variables, always use the format `{{context.${variable_name}}}`.
...
- The Canvas step analytics counts this as _Not Updated_.
...
In all circumstances, we strongly recommend using [Liquid time_zone filters](https://www.braze.com/docs/user_guide/messaging/canvas/create_a_canvas/context_and_event_properties#things-to-know) for timestamps to be represented in the desired time zone. You can reference this [frequently asked question](#faq-example) for an example.
...
|---|---|---|
| ```{{canvas_entry_properties.${timestamp_property}}}``` | `2025-08-05T08:15:30:250-0800` | No |
| ```{{canvas_entry_properties.${timestamp_property} | date: "%Y-%m-%d %l:%M %p"}}``` | `2025-08-05 4:15pm` | No
| ```{{canvas_entry_properties.${timestamp_property} | time_zone: "America/Los_Angeles" | date: "%Y-%m-%d %l:%M %p"}}``` | `2025-08-05 8:15am` | Yes |
