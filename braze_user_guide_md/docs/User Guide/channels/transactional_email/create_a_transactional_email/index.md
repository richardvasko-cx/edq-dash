...
> Braze Transactional Emails are sent to facilitate an agreed-upon transaction between a sender and the recipient. This reference article covers how to create a transactional email campaign in the Braze dashboard and generate a `campaign_id` to include in your API calls for our [`/transactional/v1/campaigns/{campaign_id}/send` endpoint](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_transactional_message).
...
Transactional Email API sends support Message Archiving. If message archiving is enabled for email in your workspace, Braze saves a rendered copy of each transactional email send. For more information, see [Message archiving](https://www.braze.com/docs/user_guide/data/distribution/export_braze_data/message_archiving/).
...
![Create Campaign dropdown with the highlighted option for transactional email.](https://www.braze.com/docs/assets/img/transactional_email_campaign.png?35c766c946e547bee493a43cb399958a){: style="float:right;max-width:35%;margin-left:15px;"}

Now, you can move on to configuring your transactional email campaign.

## Step 2: Configure your campaign

The campaign creation flow for Transaction Email campaigns is simplified compared to that of a [standard email campaign](https://www.braze.com/docs/user_guide/channels/email/html_editor/) to ensure your business-critical transaction email can reach all users.
...
![Compose, Delivery, and Confirm workflow to create a Transactional Email campaign.](https://www.braze.com/docs/assets/img/transactional_campaign_compose.png?e82a89a3e130e65aca93871cfafae85a){: style="max-width:80%;"}
...
3. Take note of your `campaign_id`. After you save your API campaign, you must include the generated `campaign_id` fields with your API request where noted in the [Transactional Email endpoint](https://www.braze.com/docs/api/endpoints/messaging/send_messages/post_send_transactional_message) article.
4. Click **Save Campaign**, and you're set to begin your API campaign!

**Note:**


The one-click list-unsubscribe setting for transactional email campaigns defaults to **Use workspace default**, similar to other email campaigns. Since this is intended for transactional messaging, Braze doesn't add one-click unsubscribe. To add a one-click unsubscribe to this campaign type, [edit this setting](https://www.braze.com/docs/user_guide/administer/global/workspace_settings/email_preferences/#message-level-one-click-list-unsubscribe) under **Sending Info**.
