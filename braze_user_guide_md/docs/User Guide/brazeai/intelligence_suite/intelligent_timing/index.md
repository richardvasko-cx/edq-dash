# [![Braze Learning course](https://www.braze.com/docs/assets/img/bl_icon3.png?5f6465f63e399dec15d7020b6f4d2452)](https://learning.braze.com/intelligent-timing){: style="float:right;width:120px;border:0;" class="noimgborder"}Intelligent Timing
...
- Email Clicks
- Email Opens (excluding [Machine Opens](https://www.braze.com/docs/user_guide/analytics/metrics_glossary#machine-opens))
- SMS Clicks (only if [link shortening](https://www.braze.com/docs/user_guide/channels/sms_mms_and_rcs/message_features_and_optimization/link_shortening/) and advanced tracking are enabled)
...
![Campaign scheduling screen showing Intelligent Timing with fallback time and quiet hours settings](https://www.braze.com/docs/assets/img/intelligent_timing/campaign_scheduling.png?a2c851b51abca64366e12363f5b0853a)
...
![Delivery preview chart for Android Push showing peak engagement time between 12 to 2 PM, and the most popular app time being 2 PM.](https://www.braze.com/docs/assets/img/intel-timing-preview.png?51ee4d83ec571b41eedfd0d6a026112e)
...
![Filters for the target audience where the campaign targets users who first used app between 1 and 4 days ago.](https://www.braze.com/docs/assets/img/intelligent_timing/first_used_app.png?8b858d1546e734aa0a65f8a0810ae437)
...
If you are leveraging [A/B testing with an optimization](https://www.braze.com/docs/user_guide/messaging/ab_testing/optimizations/), such as automatically sending the **Winning Variant** or using a **Personalized Variant**, Intelligent Timing may affect the duration and timing of your campaign.
...
![A/B testing sections showing A/B test with Winning Variant selected, with winning criteria, send date, and local send time selected](https://www.braze.com/docs/assets/img/intelligent_timing/ab_testing_intelligent_timing.png?7bef5e9cbf33b45c8bd5b0c0967eda58)
...
Choose a fallback time to use if a user's profile doesn't have any relevant events to calculate an optimal delivery time.

![Scheduling a campaign with Intelligent Timing](https://www.braze.com/docs/assets/img/intelligent_timing_1.png?1a691dbab013f9a02aabdc71d1ddc9f0)
...
In your Canvas, add a [Message step](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/message_step/), then go to **Delivery Settings** and select **Using Intelligent Timing**.
...
Choose a fallback time for the message to send to users in your audience who don't have any relevant engagement data for Braze to calculate an optimal send time. You can choose one of the following options:
...
Instead, add a [Delay step](https://www.braze.com/docs/user_guide/messaging/canvas/canvas_components/delay_step/) of at least two calendar days between the user entering the Canvas and when they receive the Intelligent Timing step.
...
![Graphic depicting the difference between days and calendar days, where if a user's optimal time is 2 pm, but they enter the delay step at 2:01 pm, and the delay is set to 2 days. Days delivers the message 3 days later because the user entered the step after their optimal time, whereas calendar days delivers the message 2 days later, on the last day of the delay.](https://www.braze.com/docs/assets/img/intelligent_timing_daysvcalendardays.png?6b20bd5929b46ed01ca4e34f06045b81){: style="border:none;"}
...
If there aren't any relevant events for a user (for example, new users with little to no engagement), Braze uses the configured fallback setting—either your custom fallback time or the most popular time to use the app among all users.
...
Your Intelligent Timing campaign might be sending past the scheduled date if you are leveraging [A/B testing with an optimization](https://www.braze.com/docs/user_guide/messaging/ab_testing/optimizations/). Campaigns using A/B testing optimizations can automatically send the Winning Variant after the initial test is over, increasing the duration of the campaign. By default, campaigns with an optimization will send the Winning Variant to the remaining users the day after the initial test, but you can change this send date.
...
No, [Machine Opens](https://www.braze.com/docs/user_guide/analytics/metrics_glossary#machine-opens) are excluded from calculations for optimal time. This means that send times are based solely on genuine user engagement, providing more accurate timing for your campaigns.
...
#### What are the fallback calculations if there are no relevant events?

If there are no relevant events for a user, Intelligent Timing uses the configured fallback setting in your message settings—either a custom fallback time or the most popular time to use the app among all users. 
...
If there aren't any relevant engagement events for a user (for example, new users with little to no clicks or opens), Intelligent Timing uses the configured fallback setting—either your custom fallback time or the most popular time to use the app among all users.
